...
class FollowUser(LoginRequiredMixin, View):
    ...
    def post(self, request, follow_unfollow, user_id):
        ...
        users = self.get_users(
            request, request.POST['rechercher_un_utilisateur'])
        return render(
            request,
            'authentication/follow_users.html',
            context={
    ...
    def get_users(self, request, filter=None):
        ...
        If the method is POST, that means that there is a filter,
        entered by the user in an input box. That will filter
        the query to find the users with a username containing the
        filter.
        It buils 2 queryset, 1 with the followed users and the other
        with everyone else.
        """
        if filter:
            followed_users = request.user.followed_users.filter(
                username__contains=filter)
            all_users = models.User.objects.filter(
                username__contains=filter).exclude(username=request.user)
        else:
            followed_users = request.user.followed_users.all()
            all_users = models.User.objects.all().exclude(
